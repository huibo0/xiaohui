...
/** 从 DB 读取昵称，默认"小白" */
function pillName(): string {
  try {
    return getSetting('pillNickname') || '小白';
  } catch {
    return '小白';
  }
}
...
export async function sendFirstReminder(period: 'morning' | 'evening'): Promise<{ success: boolean; error?: string }> {
  ...
  const greetings = {
    morning: [
      '早安呀~先给宝宝喂奶，喂完记得吃${name}哦',
      '早上好，先喂宝宝，然后吃${name}开启新的一天',
      '太阳出来啦，先喂奶再吃${name}哦~',
      '早安，宝宝饿了先喂奶，喂完别忘吃${name}呀',
    ],
    evening: [
      '辛苦了一天~先喂宝宝，喂完记得吃${name}哦',
      '晚上好，先给宝宝喂奶，然后吃${name}休息~',
      '今天也辛苦啦，先喂奶再吃${name}呀',
      '晚安前先喂宝宝，喂完记得吃${name}哦',
    ],
  };
  const msgs = greetings[period];
  const greeting = msgs[Math.floor(Math.random() * msgs.length)].replace(/\$\{name\}/g, name);
  ...
  return sendTemplateMessage(openId, templateId, {
    ...
    remark: { value: `先喂奶，喂完再吃${name}~ 点这里记录 💊`, color: '#999999' },
  }, siteUrl());
}
...
export async function sendFollowupReminder(period: 'morning' | 'evening'): Promise<{ success: boolean; error?: string }> {
  ...
  const msgs = [
    `喂完奶了吗？${periodName}的${name}还没吃呢~`,
    `宝宝吃饱了吧？该吃${name}啦，别忘了哦`,
    `提醒一下，${periodName}的${name}还没确认吃哦`,
  ];
...
export async function sendFinalReminder(period: 'morning' | 'evening'): Promise<{ success: boolean; error?: string }> {
  ...
  return sendTemplateMessage(openId, templateId, {
    first: { value: `最后一次提醒啦，${periodName}的${name}一定要吃哦！`, color: '#ef4444' },
    keyword1: { value: name, color: '#333333' },
    keyword2: { value: nowTimeStr(), color: '#333333' },
    remark: { value: `为了宝宝和自己的健康，记得按时吃${name}~ 点这里确认 💊`, color: '#999999' },
  }, siteUrl());
}
